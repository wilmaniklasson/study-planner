describe('End-to-end tests for task management', () => {
  ...
  it('should manage tasks end-to-end', () => {
    ...
    // Starta om veckan och verifiera att uppgifterna återställs
    cy.contains('Starta om vecka').click();
    cy.get('[data-cy="checkbox"]').each((checkbox) => {
      expect(checkbox).not.to.have.class('done'); // Förväntar sig att inga uppgifter är avklarade
    });
    
    // Verifiera att alla uppgifter fortfarande finns kvar efter omstart
    cy.contains('Övning 1').should('be.visible');
    cy.contains('Övning 2').should('be.visible');
    cy.contains('Övning 3').should('be.visible');
    cy.contains('Övning 4').should('be.visible');
